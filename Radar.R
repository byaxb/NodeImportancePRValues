ggradar2 <- function (plot.data, base.size = 15, font.radar = "Times New Roman",
                      ...
                      legend.text.size = 14)
{
    ...
    base <- base + theme(legend.key.width = unit(3, "line")) +
        ...
        theme(legend.text = element_text(size = legend.text.size),
              legend.position = "bottom") + scale_colour_manual(values = colour_values) +
        theme(text = element_text(family = "Times New Roman", size = 2)) + theme(legend.title = element_blank())
    if (plot.title != "") {
...
pr_value %>%
    ...
    t() %>%
    as.data.frame() %>%
    rownames_to_column('metrics') -> pr_radar

old_new_words <- read.csv("node_en.csv",
                          sep = ',',
                          header = TRUE,
                          stringsAsFactors = FALSE)
pr_radar_colnames <- colnames(pr_radar)
for(i in 1:nrow(old_new_words)) {
    pr_radar_colnames <- str_replace(pr_radar_colnames,
                                     old_new_words[i, 'old'],
                                     old_new_words[i, 'new'])
}

colnames(pr_radar) <- byaxb::dotchar(pr_radar_colnames, 28, '..')
...
pr_radar[, 1] <- c('Partial Betweeness', "Radial Degree")
ggradar2(pr_radar,
        axis.label.size = 3,
        grid.label.size = 2,
        group.line.width = 1,
